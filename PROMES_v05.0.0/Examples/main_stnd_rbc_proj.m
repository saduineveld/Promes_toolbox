...
%  (OPTIONAL) Set stopping criteria for solver:
par.options = []; 
par.opt_acc = 1;%0: default accuracy; 1: higher accuracy
if par.opt_acc == 1    
    if strcmp(POL.algo,'spl_tmi') || strcmp(POL.algo,'cheb_tmi') ||  strcmp(POL.algo,'smol_tmi')
        % For Time Iteration:
        par.tmi_tol     = 1e-12;
...
% Solve model
POL         = solve_proj(GRID,POL,fun_res,Y0,par.options);
toc; 
clear fun_res Y0;

% Plot policy function (2D and 3D):
plot_pol_stnd_rbc(par,SS,GRID,POL)
...
% Compute EEE:
[ERR_ig_bs] = STND_RBC_proj_EEE(par,GRID.xx,GRID,POL);
...
% Compute EEE:
[ERR_bs] = STND_RBC_proj_EEE(par,G_EQ.xx,GRID,POL);
