...
if strcmp(POL.algo,'spl_tmi') || strcmp(POL.algo,'spl_dir')  || strcmp(POL.algo,'spl_fpa')
    if strcmp(spec_opt,'ini_grid') && (strcmp(POL.algo,'spl_tmi') || strcmp(POL.algo,'spl_fpa'))
        error('With spl_tmi pol. at ini. grid is POL.YY. With spl_fpa pol. at ini. grid is PP.YY_old. See Chapter Model File in the manual'); 
...
elseif strcmp(POL.algo,'smol_tmi') || strcmp(POL.algo,'smol_dir') || strcmp(POL.algo,'smol_fpa')
    if strcmp(spec_opt,'ini_grid')
        if strcmp(POL.algo,'smol_tmi') 
            error('With smol_tmi pol. at ini. grid is POL.YY. With smol_fpa pol. at ini. grid is PP.YY_old. See Chapter Model File in the manual'); 
        else
...
elseif strcmp(POL.algo,'cheb_tmi') || strcmp(POL.algo,'cheb_fpa')
    if strcmp(spec_opt,'ini_grid')
        error('With cheb_tmi pol. at ini. grid is POL.YY. With cheb_fpa pol. at ini. grid is PP.YY_old. See Chapter Model File in the manual'); 
        %YY      = GRID.XX_poly_dw*POL.theta(:,i_pol);
    else
        xx_dw   = scal_mat_dw(xx,GRID.lb,GRID.ub);
